...
class Html
{
    ...
    public static function input($type,$value,$option)
    {
        $option = array_merge($option,['type'=>$type,'value'=>$value]);
        return Html::tag('input',null,$option);
    }


    public static function radio($value,$current = null,$option = [])
    {
        if ($current === $value) {
            $option[] = 'checked';
        }
        return Html::input('radio',$value,$option);
    }
